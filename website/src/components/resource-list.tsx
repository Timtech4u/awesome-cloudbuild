...
import type { Section } from "@/lib/parse-readme";
import {
...
const iconMap: Record<string, React.ElementType> = {
  "useful-links": Link, "whats-new-since-2021": Sparkles, quickstarts: Rocket,
  tutorials: BookOpen, "security--supply-chain": Shield,
  "repositories-triggers--source-integrations": GitBranch,
  "private-pools--regional-builds": Lock, "cloud-deploy--delivery": Cloud,
  "aiml--troubleshooting": Brain, "migration--deprecations": ArrowRight,
  articles: FileText, "cloud-builders": Hammer,
  "community-cloud-builders": Users, "configuration-file-templates": Code,
  tools: Wrench, meetups: Users,
};
...
function getIcon(id: string): React.ElementType {
  ...
  for (const [key, icon] of Object.entries(iconMap)) {
    if (id.includes(key) || key.includes(id)) return icon;
  }
  return Link;
}
...
export function ResourceList({ sections }: { sections: Section[] }) {
      ...
      <div className="mx-auto max-w-7xl flex">
        ...
        <main className="flex-1 min-w-0 px-4 py-6 lg:px-8">
          ...
          <footer className="pb-8 text-center text-sm text-muted-foreground space-y-1">
            <p>
              Curated by{" "}
              <a href="https://github.com/Timtech4u" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">
                Timothy Olaleke
              </a>
            ...
            <p>
              Auto-synced from{" "}
              <a href="https://github.com/Timtech4u/awesome-cloudbuild" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">
                GitHub
              </a>
              {" "}&middot; Updates every hour
            </p>
          </footer>
        </main>
      </div>
